...
class NeuralNetworkTrainer:
    ...
    def predict(self, X_test):
        """Predicts the suitability score for the provided data."""
        return self.model.predict(X_test)


def parse_labels_from_filename(filename):
    """Parses the manual labels from the filename."""
    parts = filename.split('_')
    scores = {
        'ocr': int(parts[1]),
        'color': int(parts[3]),
        'quality': int(parts[5]),
        'edges': int(parts[7].split('.')[0])
    }
    return scores


def load_labeled_data(input_dir):
    """Loads the manually labeled data from the input directory."""
    X = []
    y = []
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            scores = parse_labels_from_filename(filename)
            X.append([scores['ocr'], scores['color'], scores['quality'], scores['edges']])
            y.append(np.mean(list(scores.values())))  # Average of the scores as the overall label
...
if __name__ == "__main__":
    ...
    input_dir = '../sample_scraped_beau_20'
    processor = ImageProcessor(config)
    metrics = processor.process_images(input_dir)

    # Load labeled data
    X, y = load_labeled_data(input_dir)
    ...
    for i, pred in enumerate(predictions):
        print(f"Predicted: {pred[0]:.2f}, Actual: {y[i]:.2f}")